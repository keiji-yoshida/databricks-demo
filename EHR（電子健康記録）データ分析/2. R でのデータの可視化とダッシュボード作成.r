...
encounters <- sql("SELECT * FROM rwd.patient_encounters")
patients <- sql("SELECT * FROM rwd.patients")
...
display(
  limit(
    arrange(sql("SELECT REASONDESCRIPTION, count(*) AS count FROM rwd.patient_encounters WHERE REASONDESCRIPTION IS NOT NULL GROUP BY REASONDESCRIPTION"),'count', decreasing = TRUE)
  ...
  limit(
    arrange(
      count(
        groupBy(
          sql(
            "SELECT DISTINCT PATIENT, REASONDESCRIPTION FROM rwd.patient_encounters WHERE REASONDESCRIPTION IS NOT NULL"
          ),
...
query <- paste("
  SELECT REASONDESCRIPTION, count('*') as count FROM rwd.patient_encounters WHERE lower(REASONDESCRIPTION) LIKE '%",
...
first_condition_patients <- (sql(paste("SELECT DISTINCT PATIENT FROM rwd.patient_encounters WHERE lower(REASONDESCRIPTION) LIKE '%", dbutils.widgets.get('first_condition'), "%'", sep = '')))
...
second_condition_patients <- (sql(paste("SELECT DISTINCT PATIENT FROM rwd.patient_encounters WHERE lower(REASONDESCRIPTION) LIKE '%", dbutils.widgets.get('second_condition'), "%'", sep = '')))
